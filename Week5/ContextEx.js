...
class Parent extends Component {
    //themeValue 라는 state 변수를 만들고 'light' 값을 지정함
    state = { themeValue: 'light' }

    //현재의 themeValue 의 값을 확인하고 'light'와 'dark'를 토글함
    toggleThemeValue = () => {
        const value = this.state.themeValue === 'dark' ? 'light' : 'dark'
        this.setState({ themeValue: value })
    }
    render() {
        //자식 컴포넌트에 context 전달(provider)
        //Provider로 감싼 모든 데이터나 함수는 Consumer로 감싼
        //자식 컴포넌트에서 참조할 수 있게 됨
        return (
